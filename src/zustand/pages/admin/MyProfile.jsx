...
import { UserIcon, ShieldCheckIcon, CheckCircleIcon, FloppyDiskIcon } from '@phosphor-icons/react'
...
export default function ProfileAdmin() {
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      <div>
        <h1 className="font-rhdis text-3xl font-black text-blam uppercase tracking-tight">Admin Profile</h1>
        <p className="text-slate-500 text-sm mt-1">Kelola identitas akun dan hak akses administrator Anda.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-blam text-white border-2 border-blam p-6 shadow-[6px_6px_0px_0px_#f3ae41] flex flex-col justify-between">
          <div>
            <div className="w-16 h-16 bg-yelam text-blam border-2 border-white flex items-center justify-center font-sky text-2xl font-bold mb-4 shadow-[3px_3px_0px_0px_#ffffff]">
              {userData?.name ? userData.name.charAt(0).toUpperCase() : 'A'}
            </div>
            <h3 className="font-rhdis font-bold text-xl text-white">{userData?.name || 'Admin'}</h3>
            <span className="inline-block mt-1 text-xs font-extrabold uppercase bg-yelam text-blam px-2 py-0.5">
              Role: {userData?.role}
            </span>

            <div className="mt-6 pt-4 border-t border-slate-700 space-y-2 text-xs text-slate-300">
              <div className="flex items-center gap-2">
                <ShieldCheckIcon size={16} className="text-yelam" />
                <span>Security Level: High</span>
              </div>
              <div className="flex items-center gap-2">
                <UserIcon size={16} className="text-yelam" />
